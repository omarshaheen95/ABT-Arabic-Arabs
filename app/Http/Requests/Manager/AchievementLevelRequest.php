...
class AchievementLevelRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($this->isMethod('post')) {
            $rules['badge_icon'] = 'required|file|mimetypes:' . allowedUploadMimetypes('image') . '|max:2048';
        } else {
            $rules['badge_icon'] = 'nullable|file|mimetypes:' . allowedUploadMimetypes('image') . '|max:2048';
        }
    ...
    public function messages()
    {
        return [
            ...
            'badge_icon.mimetypes' => t('The badge icon must be a file of type') . ': ' . implode(', ', allowedUploadExtensions('image')),
