        ...
        <div class="certificates-table-wraper">
            ...
            <table class="certificates-table">
                ...
                @foreach($student_tests as $student_test)
                    <tr>
                        <td class="lesson-name">{{class_basename($student_test->model_type)}}</td>
                        <td>{{optional($student_test->model)->name}} </td>
                        <td> {{$student_test->teacher->name ?? 'N/A'}} </td>
                        <td>
        ...
        @foreach($student_tests as $student_test)
            <div class="certificate-card">
                <div class="card-header">
                    <h3 class="card-title">{{optional($student_test->model)->name}}</h3>
                    <span class="status-badge success">{{class_basename($student_test->model_type)}}</span>
                </div>
                <p class="card-level">{{t('Teacher')}}: {{$student_test->teacher->name ?? 'N/A'}}</p>
                ...
                <div class="card-actions">
                    <button class="btn-outline glass-button-component" style="width: 100%;" onclick="previewCertificate('{{route('certificate.get-certificate',['type' => $type,'id'=>$student_test->id])}}')">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 5C7 5 2.73 8.11 1 12.5 2.73 16.89 7 20 12 20s9.27-3.11 11-7.5C21.27 8.11 17 5 12 5zm0 12.5c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z" fill="currentColor"/>
                        </svg>
                        {{t('Certificate')}}
