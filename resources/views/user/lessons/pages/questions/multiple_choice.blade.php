...
<div class="question-container" id="question{{$question->id}}" data-question-id="{{$question->id}}" data-question-type="multiple-choice" style="display: {{$loop->iteration==1?'block':'none'}};">
    <div class="question-header">
        <div class="question-text">
            <h2 class="question-title">اختر الآجابة الصحيحية</h2>
        </div>
    </div>
...
{{--                    </div>--}}
        ...
        <div class="choice-options">
            @foreach($question->options as $option)
                <div class="choice-option" data-choice-id="{{$option->id}}" data-question-id="{{$question->id}}">
                    <span class="choice-number">{{$loop->iteration}}</span>
                    @if($option->image)
                        <img src="{{asset($option->image)}}" alt="Option {{$loop->iteration}}" style="max-width: 100px; border-radius: 4px;">
                    @else
