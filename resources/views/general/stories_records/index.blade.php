        ...
        <ul class="dropdown-menu">
            @can('export user records')
                <li><a class="dropdown-item" href="#!"
                       onclick="excelExport('{{route(getGuard().'.stories_records.export')}}')">{{t('Export')}}</a></li>
            @endcan
...
@section('script')
    ...
    <script>
        var DELETE_URL = "{{ route(getGuard().'.stories_records.destroy') }}";
        var TABLE_URL = "{{ route(getGuard().'.stories_records.index') }}";
        var TABLE_COLUMNS = [
            {data: 'id', name: 'id'},
