        ...
        <ul class="dropdown-menu">
            @can('export story tests')
                <li><a class="dropdown-item" href="#!"
                       onclick="excelExport('{{route('manager.stories_tests.export')}}')">{{t('Export')}}</a></li>
...
@section('filter')
    <div class="row">
        ...
        <div class="col-lg-2 mb-2">
            <label>{{t('Level')}} :</label>
            <select name="grade" class="form-select" data-control="select2" data-placeholder="{{t('Select Level')}}"
                    data-allow-clear="true">
                <option></option>
...
@section('script')
    <script>
        ...
        $('select[name="grade"]').change(function () {
            var grade = $(this).val();
            getStoriesByGrade(grade, 'select[name="story_id"]');
        });
    </script>
