            ...
            @can('export story assignments')
                <li>
                    <a class="dropdown-item" href="#!"
                       onclick="excelExport('{{route(getGuard().'.user_story_assignment.export')}}')">{{t('Export')}}</a>
                </li>
            @endcan
...
@section('filter')
    <div class="row">
        ...
        @if(request()->get('story_assignment_id'))
            <input type="hidden" name="story_assignment_id" value="{{request()->get('story_assignment_id')}}">
        @endif
    </div>
@endsection
...
@section('script')
    ...
    <script>
        var DELETE_URL = "{{ route(getGuard().'.user_story_assignment.destroy') }}";
