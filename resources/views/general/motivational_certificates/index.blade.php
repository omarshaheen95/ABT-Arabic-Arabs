...
@section('actions')
    @can('add motivational certificate')
        <a href="{{route(getGuard().'.motivational_certificates.create')}}?cer_type=lesson" class="btn btn-primary btn-elevate btn-icon-sm me-2">
            <i class="la la-plus"></i>
            {{t('Lesson Certificate')}}
        </a>
        <a href="{{route(getGuard().'.motivational_certificates.create')}}?cer_type=story" class="btn btn-primary btn-elevate btn-icon-sm me-2">
            <i class="la la-plus"></i>
            {{t('Story Certificate')}}
        </a>
    @endcan
        ...
        <ul class="dropdown-menu">
            @can('export motivational certificate')
                <li><a class="dropdown-item" href="#!"
                       onclick="excelExport('{{route(getGuard().'.motivational_certificates.export')}}')">{{t('Export')}}</a>
                </li>
            @endcan
...
@section('script')
    <script>
        ...
        var DELETE_URL = "{{ route(getGuard().'.motivational_certificates.destroy') }}";
        var TABLE_URL = "{{ route(getGuard().'.motivational_certificates.index') }}";
        var TABLE_COLUMNS = [
            {data: 'id', name: 'id'},
