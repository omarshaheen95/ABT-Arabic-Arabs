                ...
                <div class="col-lg-12">
                    <div class="table-card">
                        <div class="table-header">
                            <div class="title"> الشهادات التحفيزية </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                @foreach($student_tests as $student_test)
                                    <tr>
                                        <td>{{class_basename($student_test->model_type) == 'Story' ? 'قصة':'درس'}}</td>
                                        <td> {{optional($student_test->model)->name}} </td>
                                        <td>{{$student_test->created_at->format('Y-m-d H:i')}}</td>
                                        <td>
                                            <div class="btn-option">
                                                <a target="_blank" href="{{route('motivational.certificate', $student_test->id)}}" class="btn btn-review">
