...
@section('content')
    <form action="{{route('manager.import_files.save_logs')}}" method="POST" id="form-data">
        {{csrf_field()}}
...
@section('script')
    ...
    {!! JsValidator::formRequest(\App\Http\Requests\Manager\ImportFileLogDataRequest::class, '#form-data'); !!}

    <script>
        var DELETE_URL = "{{route('manager.import_files.delete_logs')}}";
        var TABLE_URL = "{{route('manager.import_files.show_logs', $studentDataFile->id)}}";
        var TABLE_COLUMNS = [
            {data: 'id', name: 'id'},
        ...
        const SAVE_URL = "{{route('manager.import_files.save_logs')}}";
